...
void hello() {
 ...
 int* evt_pusher_forloop(int* par){
   ...
   for (int i=1;i<1000*1000*1;i++){  //........4*300M = 1200MB
     ...
     buffer->push( i );
     if ( (i%1000)==0){
       if(XTERM!=NULL)fprintf(XTERM,"evt_pusher_forloop %d\n", i ); 
     }//if
   }//for
      if(XTERM!=NULL)fprintf(XTERM,
	     "END_pusher_remote (forloop)  par==%d; par0==%d;  pointer==%d\n", par,par[0],(int)buffer );
 }/*****************************end of function *******************************************/
 ...
 int* evt_poper_empty(int* par){
   concurrent_queue<int> *buffer=(concurrent_queue<int>*)par;
   if(XTERM!=NULL)fprintf(XTERM,"  POP EMPTY  par==%d; pointer==%d\n", par,(int)buffer );
   int datum=0;
   int runempty=0;
   int wait=1;
    while (1==1){
    wait=MyCond.TimedWaitRelative( 300  ) ; //
    if (wait==0){ 
      if(XTERM!=NULL)fprintf(XTERM,"POPER FILE    waitrelative gone... %s\n", "" );
      break; 
    }
   }//1==1
 ...
 int* evt_poper_file(int* par){
   ...
   //STANDARD  XML  READ-------------
	...
	outfile=fopen( fname,"ab"  );//  attach,  binary
