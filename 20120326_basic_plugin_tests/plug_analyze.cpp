...
#include <stdio.h>
#include "TROOT.h"    // Main INCLUDE !!!!   //    -lCore
#include "TSystem.h"                         //    -lCore
#include "TFile.h"                           //    -lRIO
#include "TH1F.h"                            //    -lHist
#include "TCanvas.h"  //i cannot draw in???  //    -lGpad
#include "TRandom.h"                         //    -lMathCore
#include "TVector3.h"                        //    -lPhysics
#include "mut_queue.h"                       
//#include <pthread.h>
...
extern "C" {
 ...
 int* evt_analyze_empty(int* par){
   ...
   if(XTERM!=NULL)fprintf(XTERM,"evt_analyze_EMPTY  ..........................\n" );
   usleep(1000*1000); 
   int wait=1;
   while (1==1){
    wait=MyCond.TimedWaitRelative( 300  ) ; //
    if (wait==0){ 
      if(XTERM!=NULL)fprintf(XTERM,"ANALYZE     waitrelative gone... %s\n", "" );
      break; 
    }
    }//1==1
 }//= ****************************************************
 ...
 int* evt_analyze_test(int* par){
   concurrent_queue<int> *buffer=(concurrent_queue<int>*)par;
   if(XTERM!=NULL)fprintf(XTERM,"evt_analyze_remote  ..........................3\n" );
   usleep(1000*1000);
   if(XTERM!=NULL)fprintf(XTERM,"evt_analyze_remote  ..........................2\n" );
   usleep(1000*1000);
   if(XTERM!=NULL)fprintf(XTERM,"evt_analyze_remote  ..........................1\n" );
