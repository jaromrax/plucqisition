...
###	&Log( "   #ActiveThread $t    #Starting server $t");
###	&Log( "   #Stopping server $t #leaving ActiveThread #$t ");
##   CLIENT THREAD
#    }else{ 

#	sleep 1;
	&Log( "   #ActiveThread :     #Starting client [$j]           ");
#	# DEAL with the file that comes by network....PCxxxx
#	if ( -e "$PCfname[$t]$online_ext" ){
#	 	&Log( "   #FILE $PCfname[$t]$online_ext exists
#           moving to   $PCfname[$t]$number.orphan");   
#   	      `mv  $PCfname[$t]$online_ext   $PCfname[$t]$number.orphan`;
#	}

#	print " starting listen nc  t== $t ...  $IP[$t] saving to   $PCfname[$t]$online_ext  \n";
...
#	&Log( "   #CMD $nc  $IP[$t]  $port_datauni >  $PCfname[$t]$online_ext ");
#	` $nc  $IP[$t]  $port_datauni >  $PCfname[$t]$online_ext`;

#	print " has quit   nc  $t ...  $IP[$t]  , about to MV  \n";
	 # number  contains  startpu time
#	`mv  $PCfname[$t]$online_ext   $PCfname[$t]$number`;

	&Log( "   #Stopping client [$j] #leaving ActiveThread # ...");

## this can be in translation     ... file $PCfname[$t]$number");

#    }# CLIENT  END

    # &Log( "   #leaving ActiveThread # $t");
}#Activate Thread
...
sub INITbut{
 ...
 for ($j=0;$j<$maxpids;$j++){ 
  if ($chk_cam[$j]==1){
       my $cmdline=get_xml_data( $j, "init" , "translate" );
...
############ $labelT2->configure( -text => ""  );### we can change the status text
 ...
 for ($j=0;$j<$maxpids;$j++){ 
  if ($chk_cam[$j]==1){
       my $cmdline=get_xml_data( $j, "start" , "translate" );
...
sub Listenbut{
   my $par=shift;
	 ...
	 if ($filepid10_onoff[$par]==0){	
	   ...
	   &Log("S.Func# $par ON <".&get_xml_data($par,"namel").">" ); `echo 1 > $pid_fname01[$par]`;
...
#    $b_gomon->configure(-background=>'grey',-activebackground =>'grey' );
#    $b_stop->configure(-background=>'grey',-activebackground =>'grey');
#    $b_gomon->configure(-background=>'grey',-activebackground =>'grey' );
    $b_gomon->configure(-background=>'white',-activebackground =>'white' );
    $b_stop->configure(-background=>'red',-activebackground =>'red');
